...
def png_gen(txt_file, pallet_file):
    ...
    subprocess.run([exe_path, '-d', txt_file, '-p', pallet_file, '-qn', 'received_power', '-qu', 'db_mw', '-o', txt_file.replace('.txt', '.png')])

    results_dir = os.path.abspath(os.path.dirname(txt_file))
    references_dir = os.path.abspath(os.path.join(results_dir, '..', 'references'))
    png_filename = os.path.basename(txt_file.replace('.txt', '.png'))
